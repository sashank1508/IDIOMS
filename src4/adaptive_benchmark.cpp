...
std::vector<std::string> generate_skewed_query_workload(int numQueries, double popularitySkew = 0.9)
{
...
public:
        ...
        : idioms::DistributedIdiomsClient(numServers, dataDirectory, useSuffixMode)
    {
        ...
        // We need to replace the standard router with our adaptive router
        // This is a bit of a hack since we're accessing a protected member,
        // but for the demo it's simpler than modifying the base class

        adaptiveRouter = std::make_shared<idioms::AdaptiveDARTRouter>(
            numServers, 0.1, 5, 10.0, 0.1, true);

        // Here we would ideally replace the router, but since it's not accessible,
        // we'll use the adaptive router directly in our overridden methods
    }

    // Override to use our adaptive router
...
void run_benchmark()
{
    ...
    std::cout << "=== IDIOMS Adaptive Query Distribution Benchmark ===" << std::endl;

    // Initialize clients
    std::cout << "\n=== Initializing standard and adaptive clients ===" << std::endl;
    idioms::DistributedIdiomsClient standardClient(NUM_SERVERS, dataDir, true);
    AdaptiveIdiomsClient adaptiveClient(NUM_SERVERS, dataDir, true);
    ...
    std::vector<std::string> workload = generate_skewed_query_workload(NUM_QUERIES, 0.8);

    std::cout << "Generated " << NUM_QUERIES << " queries with 80% bias towards popular keys" << std::endl;
